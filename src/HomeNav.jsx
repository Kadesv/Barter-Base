...
function HomeNav({ props, navRef }) {
  ...
  const onTabClick = (value) => {
    ...
    if (activeTab !== value && value !== 'Close') {
      setActiveTab(value)
      setShowDrawer(true)
    }
    else {
  ...
  return (
    <nav ref={navRef} className="navbar bg-base-300 fixed h-10 md:h-14  top-0 z-20">
      ...
      <section className="z-10 drawer drawer-end navbar-end mr-10 ">
        <input id="my-drawer-2" readOnly type="checkbox" className="drawer-toggle" checked={!!showDrawer} />

        <section className=" gap-3 flex ">
        {/*sidebar opening buttons*/}
          <label htmlFor="my-drawer-2" onClick={() => onTabClick('Favorites')} className="drawer-button btn btn-ghost btn-circle">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8">
          ...
          <label htmlFor="my-drawer-2" onClick={() => onTabClick('Chats')} className="drawer-button btn btn-ghost btn-circle">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" >
